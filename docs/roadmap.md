...
| ✅ P2 | **Vier-Bereiche-PWA** (Dashboard/Alarme/Steuerung/Einstellungen), installierbar auf Android & iOS, Tab-Leiste mit Safe-Area und Zähler offener Alarme | `app/layout.tsx`, `components/TabLeiste.tsx`, `public/manifest.webmanifest` |
...
| ⏳ P1 | **Quittierungs-getriebene Nacht-Eskalation mit lokalem Weckkanal** — bleibt ein dringender Alarm N Minuten unquittiert: Push wiederholen → zweiter Empfänger → **lokaler Weckton per MQTT**. Marktbefund August 2026: *niemand* eskaliert auf ausbleibende Reaktion; der Autodialer mit Funk-Summer ist der ehrlichste Vergleichsmaßstab für den Nacht-Alarm | Beide Hälften sind gebaut (Quittierung in der App, MQTT-Ausgang im Agenten) — es fehlt die Kette. **Eskalationstimer gehört an den Edge**, nicht in die Cloud: Erreicht der Agent die App nicht, gilt der Alarm als unquittiert und die Eskalation läuft (Fehlerfall führt zum Wecken, nicht zum Schweigen). Begründung: `wettbewerbsanalyse.md` §4 |
