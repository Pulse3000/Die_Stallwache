...
| ✅ Infra | **Tuya-only: Bridge vollständig abgelöst** — auch die Stallwache (vorher Tapo/RTSP) läuft über die Tuya-Cloud. Kein Gerät im Stall, keine Portfreigabe, kein Cloudflare Tunnel; WebRTC-/go2rtc-/MediaMTX-Pfad aus Web- und Android-App entfernt | `app/api/stallwache/stream/`, `lib/config.ts`, `lib/tuya.ts`, `components/CameraStream.tsx`; Legacy-Bridge nach `_archiv/`; Android: `Der-Stallblick` (KameraKonfig/KameraStreamView) |
| ✅ P2 | Tuya-Futterwache-Frontend (HLS, damals noch mit Bridge-Fallback) | `components/CameraStream.tsx`, `lib/tuya.ts`; nur noch `TUYA_*`-Env setzen |
| ⬜ Infra | ~~MediaMTX als Bridge-Alternative zu go2rtc (WHEP-Standard)~~ — mit der Tuya-Umstellung entfallen; Konfiguration liegt im Archiv | `_archiv/bridge/mediamtx/`, `_archiv/bridge/README.md` |
...
| ⬜ Infra | ~~Bridge auf Android/Termux~~ und ~~deren Ein-Befehl-Installer~~ — mit der Tuya-Umstellung entfallen; Skripte liegen im Archiv | `_archiv/bridge/termux/` |
